...
control 'C-1.1.2.5.4' do
  ...
  # host_lifecycle axis (#4): assert noexec where /var/tmp is a distinct mount; ephemeral
  # renders N/A when /var/tmp is folded into root. See PostureRouting#fs_na?.
  if fs_na?('/var/tmp')
    impact 0.0
    describe '/var/tmp noexec isolation N/A (host_lifecycle=ephemeral; folded into root)' do
